...
class Versions {
  ...
  static isInRange(version, range) {
    ...
    if (hor_match) {
      return (
        Versions.compare(hor_match[1], version) <= 0 &&
        Versions.compare(hor_match[2], version) > 0
      )
    }
    const hol_match = range.match(halfopen_left_range)
    if (hol_match) {
      return (
        Versions.compare(hol_match[1], version) < 0 &&
        Versions.compare(hol_match[2], version) >= 0
      )
    }
