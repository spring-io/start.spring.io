...
import { isInRange, rangeToText } from '../utils/Version'
...
function reduceDependencies(boot, items) {
  const groups = []
  const list = []
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    const valid = dep.versionRange ? isInRange(boot, dep.versionRange) : true
    if (!valid) {
      message = `Requires Spring Boot ${rangeToText(dep.versionRequirement)}.`
    }
