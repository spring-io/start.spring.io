...
/**
 * Configuration for generation of projects that depend on RabbitMQ.
 *
 * @author Moritz Halbritter
 */
@Configuration(proxyBeanMethods = false)
@ConditionalOnRequestedDependency("amqp")
class RabbitProjectGenerationConfiguration {

	@Bean
	@ConditionalOnRequestedDependency("docker-compose")
	DockerComposeService rabbitDockerComposeService() {
		DockerImages.DockerImage image = DockerImages.rabbit();
		return new DockerComposeService("rabbitmq", image.image(), image.tag(), image.website(),
